...
void Client::_sendNextFileChunk() {
    ...
    // Check for completion or error conditions first
    if (!_body.is_open() || _totalBytesSent >= _bytesToSend) {
        if (_body.is_open()) _body.close();
        _state = DONE;
        return;
    }

    // 1. If the current chunk buffer is empty, read a new one from the file.
    if (_chunkBytesSent >= _fileChunkBuffer.size()) {
        ...
        if (bytesRead <= 0) { // Error or end of file
            _body.close();
            _state = DONE;
            return;
        }
        
        _fileChunkBuffer.resize(bytesRead); // In case we read less than intended
        _chunkBytesSent = 0; // Reset the sent counter for the new chunk
    }

    // 2. Try to send the remaining data from our buffer.
    if (_chunkBytesSent < _fileChunkBuffer.size()) {
        size_t bytesToSendFromChunk = _fileChunkBuffer.size() - _chunkBytesSent;
        
        ssize_t bytesSent = send(_fd, &_fileChunkBuffer[_chunkBytesSent], bytesToSendFromChunk, 0);

        if (bytesSent > 0) {
            _chunkBytesSent += bytesSent;
            _totalBytesSent += bytesSent;
        } else if (bytesSent < 0) {
            // This is where your "chihaja" error was happening
            if (errno != EAGAIN && errno != EWOULDBLOCK) {
                // A real error occurred, so we disconnect the client.
                _body.close();
                _state = DONE;
            }
            // If it's EAGAIN, we do nothing. We'll be called again by epoll when the socket is ready.
            // The unsent data remains safe in _fileChunkBuffer.
        }
    }
    
    // 3. Final check to see if we've finished sending everything.
